...
import { coursePayment } from '../services/operations/studentApi.js';
...
function CourseDetails() {
    ...
    const handleBuyCourse = () => {
        ...
        if (token) {
            coursePayment(token, [courseId], user, navigate, dispatch);
            return;
        }
                        ...
                        <div className='py-4'>
                            {
                                courseContent.map((section) => (
                                    <div key={section._id} className='overflow-hidden border border-solid border-richblack-600 bg-richblack-700 text-richblack-5 last:mb-0'>
                                        ...
                                        {/* SubSections */}
                                        <div
                                            style={{ height: isActive.includes(section._id) ? `${section.subsection.length * 88}px` : '0px' }}
                                            className={`relative h-0 overflow-hidden bg-richblack-900 transition-[height] duration-[0.35s] ease-[ease]`}>
                                            {/* `max-h-[${section.subsection.length * 88}px]` */}
